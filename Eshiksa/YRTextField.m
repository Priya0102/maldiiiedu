...
- (void)markTextFieldsWithTagInView:(UIView*)view{
    ...
    if ([self.textFields count] == 0){
        for(UIView *subView in view.subviews){
            if ([subView isKindOfClass:[YRTextField class]]){
                YRTextField *textField = (YRTextField*)subView;
                textField.tag = index;
...
- (void) nextButtonIsClicked:(id)sender{
    ...
    YRTextField *textField =  [self.textFields objectAtIndex:++tagIndex];
...
- (void) selectInputView:(UITextField *)textField{
    if (_isDateField || _isTimeField || _isDateTimeField){
...
- (void) keyboardDidShow:(NSNotification *) notification{
    ...
    if (![_textField isKindOfClass:[YRTextField class]]) return;
